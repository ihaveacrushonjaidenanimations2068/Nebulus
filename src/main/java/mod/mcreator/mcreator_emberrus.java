...
public class mcreator_emberrus extends nebulus.ModElement {
	...
	static {
		portal = (BlockCustomPortal) (new BlockCustomPortal().setUnlocalizedName("emberrus_portal"));
		block = (ModTrigger) (new ModTrigger().setUnlocalizedName("emberrus_trigger"));
	}
	...
	public void load(FMLInitializationEvent event) {
		if (event.getSide() == Side.CLIENT) {
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation("nebulus:emberrus_trigger", "inventory"));
		}
	}

	@Override
	public void preInit(FMLPreInitializationEvent event) {
		portal.setRegistryName("emberrus_portal");
		...
		block.setRegistryName("emberrus_trigger");
		ForgeRegistries.ITEMS.register(block);
		dtype = DimensionType.register("emberrus", "_emberrus", DIMID, WorldProviderMod.class, true);
					...
					for (int k1 = -2; k1 <= 2; ++k1) {
						for (int l1 = -1; l1 < 3; ++l1) {
							...
							this.world.setBlockState(new BlockPos(i2, j2, k2),
									flag ? mcreator_refinedVsand.block.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int i8 = 0; i8 < 4; ++i8) {
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12), flag1 ? mcreator_refinedVsand.block.getDefaultState() : iblockstate, 2);
					}
				}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != mcreator_refinedVsand.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == mcreator_refinedVsand.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != mcreator_refinedVsand.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != mcreator_refinedVsand.block
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		...
		private Biome[] allowedBiomes = {mcreator_emberPlains.biome, mcreator_volcanicDeserts.biome,};
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		...
		protected static final IBlockState STONE = mcreator_emberrock.block.getDefaultState();
		protected static final IBlockState GRAVEL = mcreator_emberrock.block.getDefaultState();
		protected static final IBlockState WATER = mcreator_lichenium.block.getDefaultState();
